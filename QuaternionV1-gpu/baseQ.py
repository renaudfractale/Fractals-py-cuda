...
def create_image_array(kernel, Xinit, Yinit, Zinit, pas, max_iter, nbPoint, qw, qx, qy, qz, *args):
    pas = abs(pas)

    image_array = np.zeros((nbPoint, nbPoint, nbPoint), dtype=np.uint16)

    run_kernel(kernel, image_array, Xinit, Yinit,
               Zinit, pas, max_iter, nbPoint,qw, qx, qy, qz, *args)
...
def create_image(kernel,
    ...
    image_array = create_image_array(
        kernel, xmin, xmax, ymin, ymax, max_iter, base_accuracy, splits, *args)
...
def run_kernel(kernel, image, Xinit, Yinit, Zinit, pas, max_iter, nbPoint,qw, qx, qy, qz, *args):
    start = timer()

    dimage = cuda.to_device(image)
    threadsperblock = (32, 16)
    blockspergrid = (math.ceil(
        image.shape[0] / threadsperblock[0]), math.ceil(image.shape[1] / threadsperblock[1]))

    kernel[blockspergrid, threadsperblock](
        dimage, Xinit, Yinit, Zinit, pas, max_iter, nbPoint,qw, qx, qy, qz, *args)
